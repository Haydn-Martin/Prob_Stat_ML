...
alldataraw <- read.csv("~/Documents/Personal/R/Investing Project/Data_Extract_From_Global_Financial_Development.csv", stringsAsFactors = F)
...
# Manipulating data
  ...
  # Cumilitive return path
    ...
    # Need to separate by country, can't be stored in same matrix becasue won't be the same size
      ...
      test <- na.trim(allaret[["Argentina"]], sides = "left")
      test
      ...
      n <- 1
      countrydata <- vector("list", 88)
      cumcountrydata <- vector("list", 88)
      while (n<89) {  countrydata [[n]] <- as.data.frame(na.trim(allaret[[n]], sides = "left"))
  ...
  # Plotting 1 country
      ...
      nameSW <- "United Kingdom" # Enter name of country here
...
# Analyse data - # observations, mean, st. dev, buggest loss, biggest gain, skewness, kurtosis
  ...
  n <- 1
  lvec <- vector(length = 88)
  meanvec <- vector(length = 88)
  sdvec <- vector(length = 88)
  maxvec <- vector(length = 88)
  minvec <- vector(length = 88)
  skewnessvec <- vector(length = 88)
  kurtosisvec <- vector(length = 88)
  ...
  write.xlsx(statsdf, "~/Documents/Personal/R/Investing Project/statsdf.xlsx", sheetName = "Country Stats", col.names = T, row.names = T, append = F)
